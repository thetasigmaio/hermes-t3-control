...
Stock Hermes supports basic read/control tools. An intentional unwatched `t3_thread_send` must set `completion_policy: "none"`; the default required automatic completion fails before dispatch without the matching experimental host APIs. The optional observer requires Python `websockets` 15 and the separately published host patch for exact clean upstream commit `ad03f20dd61919ca2135d6904e787a94284aacaf`. The stock Hermes revisions tested by this release do not provide these extension APIs. The clean-base patch is separately tested; other host revisions remain unsupported. See [Experimental continuation](experimental-continuation.md).
