...
TAG_OBJECT_TYPE="$(safe_git -C "$VERIFY_DIR/repo" cat-file -t refs/tags/v1.5.0)"
...
TAG_EMBEDDED_NAME="$(safe_git -C "$VERIFY_DIR/repo" for-each-ref --format='%(tag)' refs/tags/v1.5.0)"
if [ "$TAG_EMBEDDED_NAME" != v1.5.0 ]; then
...
printf '%s\n' 'Hermes T3 Control 1.5.0 is installed and enabled.'
